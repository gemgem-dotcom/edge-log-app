...
const BASE_DIMENSIONS = {
  ...
  // Average R/expectancy are NOT degenerate for outcome:win/loss themselves
  // (average win/loss size is real signal) - just win rate. Any read-side
  // feature grouping by this dimension must not surface a win-rate figure
  // for a slice whose key contains "outcome:" - use avgR/n instead.
  outcome: (t) => {
...
// Every dimension in BASE_DIMENSIONS also doubles as a single-value groupBy
// option - i.e. all of them. Kept as an explicit derived list (not inlined
// at each use site) so it's one clear answer to "what single-dimension
// breakdowns exist."
export const SINGLE_DIMENSIONS = Object.keys(BASE_DIMENSIONS)

// N-way intersections to also groupBy - 2-way or 3-way (no cap on N itself
// in the code below, but 3-way is the deepest anyone's actually asked for;
// each extra dimension narrows the real sample size further, so going
// deeper than 3 needs a real reason, not just "because we can"). Order
// within each entry is fixed: it's the exact composite key string
// (compositeKey below joins them as 'dimA_x_dimB') that call sites like
// lib/todaysBrief.js construct directly (e.g. `strategy_id_x_${dimension}`),
// so reordering an existing entry silently breaks whoever's already asking
// for that exact key. Adding a new composite later is one more entry here;
// queryPerformance's groupBy picks it up automatically.
...
function computeRow(key, trades) {
  ...
  // reality - this fix makes expectancy a plain running average of
  // r_multiple, i.e. avgR under a different name.
