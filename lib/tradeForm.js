...
// Exit (in points) is the one mandatory field outside Trade Setup: every
// trade has to be closed, so r_multiple can always be derived. The rest of
// Trade Management stays optional. Unlike stop/target distance, negative is
// a completely valid value here (a loss), so there's no sign check.
export function validateExecution({ exit_points }) {
  const errors = {}

  if (isBlank(exit_points)) {
    errors.exit_points = 'Enter the exit in points.'
  } else if (Number.isNaN(parseFloat(exit_points))) {
    errors.exit_points = 'Exit in points must be a number.'
  }
