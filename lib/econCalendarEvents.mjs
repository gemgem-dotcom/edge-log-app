...
export function eventKey({ eventTime, currency, title, day = null }) {
  const ymd = day || ffLocalDay(eventTime) || String(eventTime).slice(0, 10)
  return `${ymd}|${String(currency || '').toUpperCase()}|${String(title || '').trim()}`
}
...
export function normalizeFeedEvent(raw) {
  ...
  return {
    ...
    detail_url: safeDetailUrl(raw.url) || detailUrlFor(parsed),
  }
}
