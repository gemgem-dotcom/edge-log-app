...
// Same tolerance FILL_PRICE_EPSILON below uses for "did this bar touch that
// price" - reused here for "did the trade's final exit leg land on its own
// stop/target", since both are float-vs-stored-numeric comparisons of the
// same kind.
...
// schema.sql's comment on why R isn't stored a second time).
//
// MFE/MAE are capped at the trade's own stop/target rather than left as
// the raw bar extreme whenever the trade's final exit leg (exitPrice)
// actually landed on that level: a stop-loss or take-profit order closes
// the position the instant price reaches it, so anything a 1-minute bar's
// high/low shows beyond that level for the same minute is intra-bar
// movement the trade was never actually exposed to. A trade that closed
// for any other reason (manual exit before either level) keeps the plain
// bar-derived figure, since nothing capped its real exposure - see
// NOTES.md for the two worked examples this was specified against.
//
// drawdown-duration uses each bar's high/low the same "did price reach
// this level" way (a bar counts as underwater if its adverse-side extreme
// crossed past entry at any point in that minute, resolving duration to
// the bar's own 60-second granularity - the best honestly available from
// minute bars) and is unaffected by the stop/target capping above: capping
// is specifically about not overstating the *extent* of an excursion past
// a level that would have closed the trade, not about when the position
// was underwater.
export function computeExcursion({ bars, entry, direction, stop, target, exitPrice }) {
