...
export function computeExcursion({ ticks, entry, direction }) {
  // A plain loop, not Math.max(...prices). Spreading an array into a call
  // passes one argument per element, and every engine caps that: past
  // roughly 100k-125k arguments V8 throws "Maximum call stack size
  // exceeded". A +/-2 minute window around a fill already runs to ~24k
  // trade prints on a busy NQ session (see NOTES.md), so a trade held a few
  // hours clears that cap comfortably - and the failure is a thrown
  // RangeError, not a wrong number: a 500 from the backfill route, and in
  // the retry job a trade caught per-trade and left 'pending' forever,
  // re-fetched every hour for eternity. One pass also beats building two
  // intermediate arrays.
...
// What an MFE/MAE/drawdown cell should actually display.
//
// realValue is whatever the caller already computed for a 'complete'
// trade, null otherwise; a 'complete' trade whose fill could not be
// verified (excursion_fallback) is treated as not having a real value
// either - see excursionStatusMessage above. A null market_data_status
// (never attempted, or not an NQ-family trade) falls through to the same
// plain "\u2014" every other not-yet-applicable field uses.
//
// Lived inside components/TradeLogTable.js as a module-local function,
// which meant the mobile trade list could not reuse it and had to either
// duplicate the logic or - as it did - omit these three fields entirely.
// It sits here with its siblings now, so both surfaces share one
// implementation and it is unit-testable, which it was not before.
export function excursionCell(trade, offsetHours, realValue) {
  if (trade.market_data_status === 'complete' && !trade.excursion_fallback && realValue !== null && realValue !== undefined) return realValue
  return excursionStatusMessage(trade, offsetHours) || '\u2014'
}
