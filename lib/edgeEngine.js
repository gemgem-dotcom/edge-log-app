...
// file directly (plain Node, outside Next.js's bundler) to get the same
// slice-membership list lib/edgeBeliefs.js's applyExcursion/reverseExcursion
// use, rather than keeping its own copy - see that script's own comment on
// why the excursion math itself is duplicated but this isn't. That only
// works because this file (and everything it imports) is plain,
// framework-free JS - no Next.js-specific imports (lib/supabaseClient.js,
// anything under app/ or components/), no JSX, no CSS. Keep it that way,
// or that script's hourly run breaks silently the next time it fires.
//
...
// N-way intersections to also slice/groupBy - 2-way or 3-way (no cap on N
// itself in the code below, but 3-way is the deepest anyone's actually
// asked for; each extra dimension narrows the real sample size further,
// so going deeper than 3 needs a real reason, not just "because we can").
// Order matters in two ways: it's the order the composite slice_key's
// segments appear in ('dimA:X|dimB:Y|dimC:Z', matching lib/edgeBeliefs.js's
// slice_key convention), and the parent a brand-new composite slice seeds
// its Bayesian prior from is always the same combination with the LAST
// dimension dropped - a 2-way's parent is dimA's own single-dimension
// slice (the N=2 case of this rule), a 3-way's parent is the 2-way built
// from its first two dimensions. So outcome before discipline means a
// fresh outcome:loss|discipline:flagged slice starts from "how do my
// losses generally look," and strategy_id|day_of_week|outcome's parent is
// strategy_id|day_of_week (the strategy's day-of-week pattern), not a
// flat guess. strategy_id leads its composites because "where does THIS
// strategy work" is the more useful starting point than "how does this
// condition generally perform, across every strategy." session leads
// day_of_week for the same kind of call - market structure as the primary
// axis, personal timing refining it - though both sides there are already
// independently informative, so that one's a closer call than the others.
// Adding a new composite later is one more entry here; queryPerformance's
// groupBy and edge_beliefs' slice list both pick it up automatically, not
...
export const COMPOSITE_SLICES = [
  ...
  ['strategy_id', 'discipline'],
  ['instrument_id', 'session'],
  ['instrument_id', 'day_of_week'],
  ['instrument_id', 'discipline'],
  ['session', 'day_of_week'],
  ['outcome', 'discipline'],
  ['strategy_id', 'day_of_week', 'outcome'],
  ['strategy_id', 'session', 'outcome'],
  ['strategy_id', 'volatility_regime', 'outcome'],
]

function compositeKey(dims) {
  return dims.join('_x_')
}

function compositeValue(dims, t) {
  const values = dims.map((d) => BASE_DIMENSIONS[d](t))
...
export const DIMENSIONS = {
  ...BASE_DIMENSIONS,
  ...Object.fromEntries(
    COMPOSITE_SLICES.map((dims) => [compositeKey(dims), (t) => compositeValue(dims, t)])
  ),
}
