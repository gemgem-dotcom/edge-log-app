...
// One request per this many trades, rather than either one request per
// trade or every trade in a single request - see the paragraph below
...
// This used to be one UPDATE request per trade, sent one at a time: fine
// for a handful of trades, but a trader with a large history could mean
// thousands of sequential round-trips. Every result is now collected into
// one array first, then sent as a handful of bulk upserts instead - each
// upsert's onConflict:'id' means it's really just a batched UPDATE (the
// row always already exists, since its id was just read from this same
// table), touching only the two columns actually passed in. Still split
// into chunks rather than one request for the whole array: a single
// request carrying many thousands of rows gets slow and fragile - one
// dropped connection loses the entire batch - whereas a failed chunk here
// only costs that chunk, and each individual request stays a predictable
// size.
export async function backfillOwnTradeSessions() {
  ...
  for (let i = 0; i < updates.length; i += BACKFILL_CHUNK_SIZE) {
    const chunk = updates.slice(i, i + BACKFILL_CHUNK_SIZE)
    // Not awaited-and-checked for error, same as the per-trade loop this
    // replaced - fire-and-forget applies to failures too, not just
    // completion, and the next chunk still gets a chance either way.
    await supabase.from('trades').upsert(chunk, { onConflict: 'id' })
  }
}
