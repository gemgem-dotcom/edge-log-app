...
// a glance" cards (economic events), until a paid market-data API
// replaces it. Every function returns the same shape the real version
...
// Every upcoming event within `windowMs` of `fromDate` (default 24h),
// ordered soonest first - same underlying search as nextEconEvent (this
// week and next, since MOCK_ECON_WEEK repeats weekly) but collecting every
// match in range instead of stopping at the first.
export function upcomingEconEvents(fromDate = new Date(), windowMs = 24 * 60 * 60 * 1000) {
  const monday = new Date(fromDate)
  const weekday = monday.getDay()
  monday.setDate(monday.getDate() + (weekday === 0 ? -6 : 1 - weekday))
  monday.setHours(0, 0, 0, 0)

  const cutoff = new Date(fromDate.getTime() + windowMs)
  const events = []
  for (const weekOffset of [0, 1]) {
    for (const e of MOCK_ECON_WEEK) {
      const [hh, mm] = e.time.split(':').map(Number)
      ...
      timestamp.setDate(timestamp.getDate() + e.day + weekOffset * 7)
      timestamp.setHours(hh, mm, 0, 0)
      if (timestamp > fromDate && timestamp <= cutoff) {
