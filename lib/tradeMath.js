...
// Shared sample-size confidence gate for any stat computed over a subset of
// a trader's trades - a strategy detail page finding, or anything else that
// wants to avoid printing a precise-looking percentage off a handful of
// trades. Same three thresholds everywhere this is used, so different
// findings on the same page (or different pages) don't each invent their
// own ad-hoc cutoff and disagree about what counts as "enough."
export function sampleConfidence(n) {
  if (n < 20) return { level: 'low', label: 'too early to trust' }
  if (n < 50) return { level: 'medium', label: 'early signal' }
  return { level: 'high', label: 'enough to trust' }
}
