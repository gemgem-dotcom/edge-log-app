...
import { parseCalendarHtml, parseClockMinutes, parseDateLabel, weekUrl, monthUrl } from './econCalendarHtml.mjs'
...
// The bound the dateline arithmetic could not cross, and how FF's own
// printed date crosses it.
//
// A dateline is midnight in FF's display zone, and FF picks that zone from
// the caller's IP. Read as UTC-date-of-dateline+12h it is right for every
// |offset| <= 12 - but the SAME epoch is midnight of two different days
// depending on which side of the dateline the zone sits, and nothing in the
// number says which. 2026-01-14T11:00Z is midnight on Jan 15 for a caller
// served UTC+13, and midnight on Jan 14 for one served UTC-11.
//
// Both readings are legitimate, so this is not a bug to compute away - it
// is missing information, and the page supplies it. FF prints the date in
// the day's own date cell, which is FF stating its answer directly.
describe('dayFor: the same dateline, two legitimate days', () => {
  // Midnight of Jan 15 at UTC+13; midnight of Jan 14 at UTC-11.
  const AMBIGUOUS = Math.floor(Date.parse('2026-01-14T11:00:00Z') / 1000)

  const dayRow = (dateline, label) =>
    `<tr data-event-id="7001" data-day-dateline="${dateline}" class="calendar__row calendar__row--new-day"> `
    + (label === null ? '' : `<td class="calendar__cell calendar__date" rowspan="4"><span class="date">${label}</span></td> `)
    + '<td class="calendar__cell calendar__time">9:00am</td> '
    + '<td class="calendar__cell calendar__currency">NZD</td> '
    + '<td class="calendar__cell calendar__impact"><span class="icon icon--ff-impact-red"></span></td> '
    + '<td class="calendar__cell calendar__event"><span class="calendar__event-title">Official Cash Rate</span></td> '
    + '<td class="calendar__cell calendar__actual"><span></span></td> '
    + '<td class="calendar__cell calendar__forecast"><span></span></td> '
    + '<td class="calendar__cell calendar__previous"><span></span></td> </tr>'

  const dayOf = (dateline, label) => parseCalendarHtml(dayRow(dateline, label)).events[0].detail_url

  it('reads a UTC+13 page as the day FF printed, not the day before', () => {
    expect(dayOf(AMBIGUOUS, 'Thu <span>Jan 15</span>')).toContain('jan15.2026')
  })

  it('reads the same dateline as Jan 14 when FF printed Jan 14', () => {
    expect(dayOf(AMBIGUOUS, 'Wed <span>Jan 14</span>')).toContain('jan14.2026')
  })

  // The regression this guards: before the label, the arithmetic answered
  // Jan 14 for both, so a UTC+13 fetch keyed and linked every row on the
  // page a day early - the same failure that put 181 backfilled rows under
  // the wrong date, one zone further out.
  it('does not answer the same day for both', () => {
    expect(dayOf(AMBIGUOUS, 'Thu <span>Jan 15</span>'))
      .not.toBe(dayOf(AMBIGUOUS, 'Wed <span>Jan 14</span>'))
  })

  // The label carries no year. It never needs to: the candidates are real
  // dates either side of the dateline, so the year comes from them.
  it('crosses a year boundary on the label alone', () => {
    const nye = Math.floor(Date.parse('2025-12-31T11:00:00Z') / 1000)
    expect(dayOf(nye, 'Thu <span>Jan 1</span>')).toContain('jan1.2026')
  })

  // Falling back has to land exactly where the parser was before, or a
  // markup change turns into silently wrong days rather than unchanged
  // ones.
  it.each([
    [null, 'no date cell at all'],
    ['', 'an empty date cell'],
    ['Thursday', 'a weekday with no date'],
    ['Smarch 40', 'a date that is not one'],
  ])('falls back to the arithmetic given %s (%s)', (label) => {
    expect(dayOf(AMBIGUOUS, label)).toContain('jan14.2026')
  })

  // A label naming a day the dateline cannot plausibly be is a label about
  // some other row. Trusting it over the dateline would be worse than
  // ignoring it.
  it('ignores a label more than a day from the dateline', () => {
    expect(dayOf(AMBIGUOUS, 'Mon <span>Mar 3</span>')).toContain('jan14.2026')
  })
})
...
describe('parseDateLabel', () => {
  it.each([
    ...
    ['Aug 28', 7, 28],
    ['Fri, Aug 28', 7, 28],
    ['Friday August 28', 7, 28],
    ['Sep. 1', 8, 1],
    ['Jan 1', 0, 1],
    ['Dec 31', 11, 31],
  ])('reads %s', (text, month, dom) => {
    expect(parseDateLabel(text)).toEqual({ month, dom })
  })

  it.each(['', null, undefined, 'Thursday', 'All Day', 'Smarch 12', 'Aug 40', 'Aug'])(
    'returns null for %s',
    (text) => {
      expect(parseDateLabel(text)).toBeNull()
    },
  )
})
