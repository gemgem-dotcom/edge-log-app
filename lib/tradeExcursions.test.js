...
// excursionCell decides what an MFE/MAE/drawdown cell actually shows.
//
// It lived inside components/TradeLogTable.js as a module-local function,
// so it could not be imported (which is why the mobile trade list omitted
// all three fields rather than reuse it) and could not be tested. Both
// are fixed by it living here.
describe('excursionCell', () => {
  const REAL = '14.25 pts'

  it('shows the real value only when the trade is complete and verified', () => {
    expect(excursionCell({ market_data_status: 'complete' }, -5, REAL)).toBe(REAL)
  })

  // A 'complete' trade whose fill could not be verified has a value, but
  // not one worth showing as fact.
  it('falls back for a complete-but-unverified trade even with a value', () => {
    expect(excursionCell({ market_data_status: 'complete', excursion_fallback: true }, -5, REAL)).toBe('Unverified')
  })

  it('falls back when complete but the value itself is missing', () => {
    expect(excursionCell({ market_data_status: 'complete' }, -5, null)).toBe('\u2014')
    expect(excursionCell({ market_data_status: 'complete' }, -5, undefined)).toBe('\u2014')
  })

  it('reports an unavailable trade rather than showing a dash', () => {
    expect(excursionCell({ market_data_status: 'unavailable' }, -5, null)).toBe('Not available for this trade')
  })

  // A null status is "never attempted, or not an NQ-family trade", which
  // is the same not-yet-applicable em-dash every other such field uses.
  it('shows a plain dash when no attempt was ever made', () => {
    expect(excursionCell({}, -5, null)).toBe('\u2014')
    expect(excursionCell({ market_data_status: null }, -5, null)).toBe('\u2014')
  })

  // 0 is a real reading - "never went against you" - and must not be
  // swallowed by a falsy check.
  it('does not treat a zero reading as missing', () => {
    expect(excursionCell({ market_data_status: 'complete' }, -5, '0.00 pts')).toBe('0.00 pts')
  })
})
