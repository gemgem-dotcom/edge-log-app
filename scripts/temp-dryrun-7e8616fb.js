...
async function main() {
  ...
  const { data: complete, error } = await admin.from('trades').select('*').eq('id', '7e8616fb-334b-4465-8a2f-e572b634df5a')
  if (error) throw new Error(`Failed to load trade: ${error.message}`)

  log(`${complete.length} trade(s) loaded (dry run - targeting 7e8616fb only, no writes).`)
  ...
  // Deliberately empty for this dry run - the whole point is to test
  // 7e8616fb against the standard pipeline despite it normally being
  // excluded (see the real script's own copy of this set).
  ...
  for (const trade of complete) {
    ...
    try {
      ...
      // DRY RUN: log the payload that would be written, don't write it -
      // this run is only to check whether the standard pipeline can
      // resolve 7e8616fb now that tick-level data exists, per the plan
      // agreed with the user before making anything permanent.
      log(`DRY RUN would write: ${JSON.stringify({
        ...
        symbol_used: symbol,
        stype_in_used: stypeIn,
        entryInstant: entryInstant.toISOString(),
        exitInstant: exitInstant.toISOString(),
        tickCountInWindow: windowTicks.length,
      })}`)
