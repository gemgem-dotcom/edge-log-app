...
// Strictly a real, finite number. The previous version only rejected
// null/undefined/NaN, which let every string through (Number.isNaN doesn't
// coerce, so Number.isNaN('abc') is false) along with booleans and arrays.
// No caller was actually passing one - they all parseFloat first or read a
// numeric column - but the failure mode was silent and financial rather
// than loud: subtraction coerces while addition concatenates, so a stray
// '30' would compute a correct stop of 21020 and a target of 2105030 from
// the same entry. Infinity is excluded for the same reason: it propagates
// through the arithmetic instead of failing.
function isNum(v) {
