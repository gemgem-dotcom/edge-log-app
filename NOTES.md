...
days, generated by `scripts/generate_cme_holidays.py` from the `pandas_market_calendars`
`CME_Equity` calendar - the real, actively maintained source for CME-specific rules
(not just a copy of NYSE's calendar). There's no live API for this; holiday
schedules are published a year in advance and barely change, so it doesn't need a
...
rollover dates) is separate, later work.
