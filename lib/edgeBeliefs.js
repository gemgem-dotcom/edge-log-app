...
//
// CRITICAL: both always return a full {pnl_mean, pnl_m2, pnl_n} object,
// never null, even when the gate fails - every row buildSliceRow returns
// goes into ONE shared bulk upsert alongside every other slice this trade
// touches (applyOrReverse's single supabase.upsert call), and Supabase's
// upsert computes its column list as the UNION of keys across the whole
// batch. A row that omitted these keys wouldn't just skip updating them -
// on the ON CONFLICT DO UPDATE path, PostgREST fills the row's missing
// columns with NULL before writing, which violates this column's `not
// null` constraint and fails the ENTIRE upsert statement (every slice in
// the batch, not just the pnl fields) the moment a trade has both a
// strategy_id and a dollar figure - the ordinary case, not an edge one.
// So a slice that doesn't qualify writes back a harmless no-op instead of
// omitting the keys: a brand-new non-qualifying row gets explicit zeros
// (identical to the column defaults), an existing non-qualifying row
// writes back its own current values unchanged.
function pnlSeedFor(bindings, trade, parent) {
  if (!hasStrategyBinding(bindings) || trade.pnl == null) return { pnl_mean: 0, pnl_m2: 0, pnl_n: 0 }
...
function pnlUpdateFor(existing, trade, sign) {
  if (!hasStrategyBinding(existing.bindings) || trade.pnl == null) {
    return { pnl_mean: existing.pnl_mean ?? 0, pnl_m2: existing.pnl_m2 ?? 0, pnl_n: existing.pnl_n ?? 0 }
  }
...
//
// Uses trade.user_id directly rather than supabase.auth.getUser() - the
// browser call sites (the trade save/edit/delete pages) always have a
// real logged-in session either way, but scripts/backfill-edge-beliefs.js
// calls this with a service-role admin client and no session at all,
// the same reason lib/edgeBeliefs.js's applyOrReverseExcursion already
// made this switch. trade.user_id is the same value getUser() would have
// resolved to for a trade that authored it, so this isn't a behavior
// change for the existing browser call sites, just a version that also
// works server-side.
