...
//
// One honest bound: "every zone a calendar site would display in" means
// |offset| <= 12. At UTC+13 or +14 (Auckland in southern summer, Apia,
// Chatham) local midnight is 11:00Z or 10:00Z on the PREVIOUS UTC day, and
// +12h lands before the rollover, so the day comes out one early - the same
// failure described above, one zone further out. A dateline alone cannot
// tell UTC-11 from UTC+13, since both put local midnight at the same point
// in the UTC day; resolving it needs the page's own printed date label.
// Left as-is deliberately: FF derives its zone from the client IP, and the
// only clients here are GitHub runners and Vercel functions, all well
// inside the bound. If either ever moves, this is the line that breaks.
const HALF_DAY_SECONDS = 43200

function dayFor(dateline) {
  return new Date((dateline + HALF_DAY_SECONDS) * 1000).toISOString().slice(0, 10)
}
...
export function parseCalendarHtml(html) {
  ...
  const coveredFrom = sortedDatelines.length
    ? new Date(sortedDatelines[0] * 1000).toISOString()
    : null
  const coveredTo = sortedDatelines.length
    ? new Date((sortedDatelines[sortedDatelines.length - 1] + DAY_SECONDS) * 1000).toISOString()
    : null
