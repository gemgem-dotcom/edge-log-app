...
export async function fetchEconomicEvents(fromDateStr, toDateStr) {
  ...
  // A day of slack at each end, then filtered exactly by the caller with
  // eventInRange. An all-day row is stored at midnight in FF's OWN zone,
  // which can sit up to most of a day outside the window that bounds the
  // viewer's local day - so a Los Angeles trader asking for a single day
  // got a query window their own bank holiday fell outside of, and the row
  // vanished rather than merely showing on the wrong date. Widening here
  // and narrowing by day in the card is what makes the two agree.
