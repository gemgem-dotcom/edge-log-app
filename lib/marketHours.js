...
// Exported (not just used by marketStatus below) so lib/tradeSessions.js
// can label a logged trade's own entry time with the same named sessions
// this pill shows live, rather than re-deriving the boundaries a second
// time and risking the two drifting apart. Called there with a trade's
// entry (and every later minute up to its exit) rather than "now", so -
// unlike marketStatus - it has no notion of whether the market is even
// open at that minute; a trade can only exist inside market hours in the
// first place, so that's never actually a concern for either caller.
export function sessionFor(minutesOfDay) {
