...
// closed full NQ trading day, or null if there isn't yet a large enough
// trailing baseline to bucket it against.
export async function latestClosedSessionRegime(supabase) {
...
export function edgeEngineClause({ trades, strategies, regime }) {
  ...
  return `NQ's most recent session traded at ${conditionLabel}, and you've historically ${directionWord(best.delta)} on ${best.strategyName} in these conditions.`
}
