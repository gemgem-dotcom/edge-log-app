...
describe('detailUrlFor', () => {
  it('builds FF\'s own day URL in its own format', () => {
    ...
    expect(detailUrlFor(new Date('2026-09-06T14:30:00Z'))).toBe('https://www.forexfactory.com/calendar?day=sep6.2026')
  })

  // The day is FF's, not UTC's. 01:30Z is 8:30pm the PREVIOUS evening in
  // FF's display timezone, and the page files it under that evening - so
  // linking it to the UTC date sent the reader to the wrong day.
  it('files an evening release under FF\'s day, not the UTC one', () => {
    expect(detailUrlFor(new Date('2026-09-07T01:30:00Z'))).toBe('https://www.forexfactory.com/calendar?day=sep6.2026')
  })
...
// The feed's own timestamp carries FF's offset, so its day is readable off
// the string. Going through a named zone instead is what made the feed
// disagree with the page about the day - and a disagreement about the day
// is a duplicate row, since the day is a third of the key.
describe('ffDayFromStamp', () => {
  it('reads the day from the offset the stamp declares, not from UTC', () => {
    // 23:30 on the 14th behind UTC is already the 15th in UTC. FF's day is
    // the 14th, and the string says so.
    expect(ffDayFromStamp('2026-09-14T23:30:00-06:00')).toBe('2026-09-14')
    expect(ffDayFromStamp('2026-03-11T08:30:00-04:00')).toBe('2026-03-11')
  })

  it('handles a UTC stamp and optional seconds/fraction', () => {
    expect(ffDayFromStamp('2026-09-14T23:30:00Z')).toBe('2026-09-14')
    expect(ffDayFromStamp('2026-09-14T23:30Z')).toBe('2026-09-14')
    expect(ffDayFromStamp('2026-09-14T23:30:00.500-0600')).toBe('2026-09-14')
  })

  it('returns null when there is no offset to trust', () => {
    expect(ffDayFromStamp('2026-09-14T23:30:00')).toBeNull()
    expect(ffDayFromStamp('not a date')).toBeNull()
    expect(ffDayFromStamp(null)).toBeNull()
  })

  it('agrees with the page path for a release either side of local midnight', () => {
    // What the zone-free dayFor computes for a Denver page: the dateline
    // (local midnight) plus twelve hours, read as a UTC date.
    const denverMidnight = Math.floor(Date.UTC(2026, 8, 14, 6, 0, 0) / 1000)
    const pageDay = new Date((denverMidnight + 43200) * 1000).toISOString().slice(0, 10)
    const feedRow = normalizeFeedEvent({
      title: 'BOJ Gov Ueda Speaks', country: 'JPY', date: '2026-09-14T23:30:00-06:00',
    })
    expect(feedRow.event_key).toBe(`${pageDay}|JPY|BOJ Gov Ueda Speaks`)
    expect(feedRow.detail_url).toContain('sep14.2026')
    // ...while the stored instant is still the real one, on the next UTC day.
    expect(feedRow.event_time).toBe('2026-09-15T05:30:00.000Z')
  })
})
...
describe('normalizeFeed', () => {
  ...
  it('collapses a repeated record so one upsert batch can never name a key twice', () => {
    const { events } = normalizeFeed([
      at('2026-03-11T08:30:00Z', 'CPI m/m'),
      at('2026-03-11T08:30:00Z', 'CPI m/m'),
    ])
    expect(events).toHaveLength(1)
    expect(events[0].event_time).toBe('2026-03-11T08:30:00.000Z')
  })

  // This used to collapse too, on the reading that a second record for the
  // same title must be the first one rescheduled. It is usually the
  // opposite: FF lists one title twice in a day whenever the same official
  // speaks morning and evening, and collapsing lost a release outright -
  // then rewrote the surviving row's time on the next fetch, so something
  // already on screen moved. Distinct instants are now distinct events,
  // matching what the page path does with FF's own event ids.
  it('keeps two same-day releases of one title apart', () => {
    const { events } = normalizeFeed([
      at('2026-03-11T14:00:00Z', 'FOMC Member Bowman Speaks'),
      at('2026-03-11T23:30:00Z', 'FOMC Member Bowman Speaks'),
    ])
    expect(events).toHaveLength(2)
    // The earlier one keeps the bare key, so every row stored before
    // sequencing existed still matches itself and nothing needs re-keying.
    expect(events[0].event_key).toBe('2026-03-11|USD|FOMC Member Bowman Speaks')
    expect(events[1].event_key).toBe('2026-03-11|USD|FOMC Member Bowman Speaks|#1')
