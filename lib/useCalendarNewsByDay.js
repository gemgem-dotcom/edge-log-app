...
import { displayDayFor } from '@/lib/econCalendarDay'
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      for (const event of data || []) {
        ...
        // Grouped by the day the event belongs to, matching how the grid
        // builds cell.dateStr. For a timed release that is the viewer's own
        // local day - an 8:30pm Chicago print is a different calendar day
        // in London, and the badge has to sit on the day the trader sees it
        // under. For an all-day row it is FF's day instead, because that
        // row has no instant of its own to place: it is anchored to
        // midnight in FF's zone, which reads as the previous day for every
