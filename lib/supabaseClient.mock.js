...
function chain(name, rows) {
  ...
  const api = {
    ...
    // count:'exact' request with no .range() narrowing it further.
    then: (resolve) => resolve({ data: sortedRows(), error: null, count: state.wantCount ? (state.count ?? sortedRows().length) : undefined }),
  }
  return api
}
