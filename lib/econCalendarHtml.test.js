...
describe('parseCalendarHtml: days that broke it', () => {
  ...
  // What a page is allowed to speak for. The removal pass deletes rows
  // that a page did NOT mention inside this range, so getting it wrong is
  // the difference between dropping a cancelled speech and dropping a real
  // day of events.
  it('reports the exact span of days it actually parsed', () => {
    const html = twoDayPage('2026-08-28T05:00:00Z', '8:30am', 'Core PCE', '2026-08-29T05:00:00Z')
    const { coveredFrom, coveredTo, days } = parseCalendarHtml(html)
    expect(days).toBe(2)
    expect(coveredFrom).toBe('2026-08-28T05:00:00.000Z')
    // Through the END of the last day it saw, not the start of it.
    expect(coveredTo).toBe('2026-08-30T05:00:00.000Z')
  })

  // A page that rendered one of its seven days speaks for one day. Taking
  // the requested week as the range instead would let a truncated response
  // delete six days of real events.
  it('speaks only for the days it saw, not the week it asked for', () => {
    const one = parseCalendarHtml(row(dateline('2026-08-28T05:00:00Z'), '8:30am', 'Core PCE'))
    expect(one.coveredFrom).toBe('2026-08-28T05:00:00.000Z')
    expect(one.coveredTo).toBe('2026-08-29T05:00:00.000Z')
  })

  it('reports no range at all for a page with no datelines', () => {
    const { coveredFrom, coveredTo } = parseCalendarHtml('<table><tr class="calendar__row"></tr></table>')
