...
// segments: [{ id, label, value, color }], one per instrument. Ranked
// highest to lowest $ P&L, including negative and zero - replaces the donut
// this card used to show, which could only represent a losing instrument as
// a muted legend row with no arc of its own. A horizontal strip of chips
...
//
// activeId: the instrument the card's own filter is scoped to (null/'all'
// means no filter). Every instrument's total still shows regardless - only
// the non-active chips dim, so the filtered-out totals stay visible for
// comparison instead of disappearing.
export default function PnlByInstrumentList({ segments, activeId }) {
  ...
  return (
    <div className="pnl-rank-strip">
      {ranked.map((seg) => (
        <div
          className={`pnl-rank-chip ${activeId && seg.id !== activeId ? 'pnl-rank-chip-dim' : ''}`}
          key={seg.label}
        >
