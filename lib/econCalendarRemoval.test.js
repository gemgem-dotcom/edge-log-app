...
// ---------------------------------------------------------------------------
//
// The real case this exists for: FF re-listed one ADP release under a
// second event id, the upsert keys on `ff|<id>`, and production held the
// same release twice - undetected until somebody ran the inspector by
// hand.
const dupRow = (key, over = {}) => ({
  event_key: key,
  event_time: '2026-10-06T12:15:00+00:00',
  currency: 'USD',
  title: 'ADP Weekly Employment Change',
  fetched_at: '2026-09-21T15:00:00.000Z',
  ...over,
})

describe('planIdentityDedupe', () => {
  it('finds nothing in a clean set', () => {
    const out = planIdentityDedupe([
      dupRow('ff|1'),
      dupRow('ff|2', { title: 'Something else' }),
      dupRow('ff|3', { event_time: '2026-10-07T12:15:00+00:00' }),
      dupRow('ff|4', { currency: 'EUR' }),
    ])
  ...
  it('catches the exact production case and keeps the freshest row', () => {
    const out = planIdentityDedupe([
      dupRow('ff|151048', { fetched_at: '2026-09-20T04:00:00.000Z' }),
      dupRow('ff|153364', { fetched_at: '2026-09-21T15:00:00.000Z' }),
    ])
    expect(out.duplicateCount).toBe(1)
    expect(out.removable.map((r) => r.event_key)).toEqual(['ff|151048'])
    expect(out.pairs[0].keep).toBe('ff|153364')
    expect(out.pairs[0].remove).toEqual(['ff|151048'])
  })

  // The absolute instant, not the calendar day. NOTES.md is explicit that
  // the DAY is not an identity - a release near local midnight belongs to
  // two different days depending on the timezone FF served the caller, and
  // keying on it stored 174 duplicates. Two rows an hour apart are two
  // events, and must not collide here.
  it('does not treat two different instants as one release', () => {
    const out = planIdentityDedupe([
      dupRow('ff|1', { event_time: '2026-08-03T06:00:00+00:00' }),
      dupRow('ff|2', { event_time: '2026-08-03T07:00:00+00:00' }),
    ])
    expect(out.duplicateCount).toBe(0)
  })

  it('breaks a fetched_at tie deterministically rather than by scan order', () => {
    const rows = [dupRow('ff|aaa'), dupRow('ff|zzz')]
    const forward = planIdentityDedupe(rows)
    const reversed = planIdentityDedupe(rows.slice().reverse())
    expect(forward.pairs[0].keep).toBe(reversed.pairs[0].keep)
    expect(forward.removable.map((r) => r.event_key))
      .toEqual(reversed.removable.map((r) => r.event_key))
  })

  it('removes every stale row when one release has three ids', () => {
    const out = planIdentityDedupe([
      dupRow('ff|1', { fetched_at: '2026-09-19T04:00:00.000Z' }),
      dupRow('ff|2', { fetched_at: '2026-09-20T04:00:00.000Z' }),
      dupRow('ff|3', { fetched_at: '2026-09-21T04:00:00.000Z' }),
    ])
    expect(out.duplicateCount).toBe(2)
    expect(out.pairs[0].keep).toBe('ff|3')
  })

  // Over the bound it returns NOTHING removable rather than a trimmed
  // list - the same stance planRemoval takes. Dozens of collisions is a
  // parser fault, and acting on a trimmed version of a bad reading would
  // destroy data while looking unremarkable.
  it('refuses the whole set past the cap instead of trimming it', () => {
    const rows = []
    for (let i = 0; i <= MAX_IDENTITY_DUPLICATES; i++) {
      rows.push(dupRow(`ff|a${i}`, { event_time: `2026-10-06T${String(i % 24).padStart(2, '0')}:15:00+00:00` }))
      rows.push(dupRow(`ff|b${i}`, { event_time: `2026-10-06T${String(i % 24).padStart(2, '0')}:15:00+00:00` }))
    }
    const out = planIdentityDedupe(rows)
    expect(out.duplicateCount).toBeGreaterThan(MAX_IDENTITY_DUPLICATES)
    expect(out.allowed).toBe(false)
    expect(out.removable).toEqual([])
    // Still reported, so the refusal is visible rather than silent.
    expect(out.pairs.length).toBeGreaterThan(0)
  })

  it('ignores malformed rows rather than colliding them on undefined', () => {
    const out = planIdentityDedupe([
      { event_key: 'ff|1' },
      { event_key: 'ff|2' },
      null,
