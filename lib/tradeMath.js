...
// Below this tolerance, a typed exit price is treated as "the same price" as
// the stored stop/target - covers float roundtrip noise from the
// entry +/- distance arithmetic in calcStopPrice/calcTargetPrice, not
// user-facing rounding.
const ADHERENCE_EPSILON = 0.0001

// Whether the trade's actual exit matches what was planned: the stop or
// target price computed from the distances entered at setup. Anything else
// (partial, early exit, runner past target) is a deviation. Returns null
// for an open trade, so callers can render a neutral state rather than a
// false "deviated".
export function planAdherence(trade) {
  if (!isNum(trade.exit_price) || !isNum(trade.stop)) return null
  if (isNum(trade.target) && Math.abs(trade.exit_price - trade.target) < ADHERENCE_EPSILON) return 'target'
  if (Math.abs(trade.exit_price - trade.stop) < ADHERENCE_EPSILON) return 'stop'
  return 'deviated'
}
