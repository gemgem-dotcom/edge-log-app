...
export async function getTags(supabase, userId) {
  ...
  // Paged: this is the "previously used tags" suggestion list, so a
  // truncation at PostgREST's 1000-row cap quietly stops offering tags that
  // only appear on older trades.
  const { data } = await fetchAllRows((from, to) => supabase
    .from('trades').select('tags').eq('user_id', userId).order('id', { ascending: true }).range(from, to))
