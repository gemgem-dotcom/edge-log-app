...
// Phase 3's edge_beliefs read side (lib/edgeFindings.js) needs to render
// real findings against - t1-t4 above are left untouched since other UI
// work verifies against their exact values. Sessions are set directly
...
// Empty by default - populated only by whatever applyTrade/reverseTrade
// (lib/edgeBeliefs.js) actually writes during a dev session, same as the
// real table starts empty for a brand-new account.
let MOCK_EDGE_BELIEFS = []
// Same story for the AI insights cache (lib/insightsClient.js) - starts
// empty, populated only once a panel actually triggers a (mocked, in dev)
// generation.
...
if (typeof window !== 'undefined') {
  ...
  window.__MOCK_EDGE_BELIEFS__ = MOCK_EDGE_BELIEFS
...
function chain(name, rows) {
  ...
  const api = {
    ...
    // Narrow, deliberately not a real PostgREST filter-string parser -
    // only handles the one shape this app's own code actually sends
    // (lib/tradeRegimes.js's 'col.is.null,col2.is.null'): a comma-joined
    // list of `column.is.null` conditions, matched as a real OR (a row
    // passes if ANY condition matches). Extend this the same narrow way
    // if a future .or() call needs a different operator, rather than
    // building out a general-purpose filter grammar nothing else needs.
    or: (filterString) => {
    ...
    // `archived` defaults to false in schema.sql for every table that has
    // it (instruments, strategies) - applied here too, since a caller that
    // omits it (the normal case; only restore/toggle flows set it
    // explicitly) would otherwise get `undefined` on the mock row, which
    // fails any later `.eq('archived', false)` read even though the same
    // insert against the real database would read back as archived=false.
    insert: (rowsToInsert) => {
      const inserted = rowsToInsert.map((r) => ({ id: 'new-' + Math.random().toString(36).slice(2), archived: false, ...r }))
    ...
    then: (resolve) => resolve({ data: sortedRows(), error: null }),
  }
  return api
}
