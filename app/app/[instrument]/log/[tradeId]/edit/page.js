...
async function handleSubmit(e) {
  ...
  let screenshot_url = existingScreenshotUrl
  if (screenshotFile) {
...
<input name="trade_time" type="time" step="1" defaultValue={trade.trade_time} required />
  </div>
<div className="field wide">
...
<div className="field wide">
  <label>In-plan?</label>
<select name="in_plan" defaultValue={trade.in_plan ? 'yes' : 'no'}>
...
{existingScreenshotUrl && !screenshotFile && (
  <div style={{ marginBottom: '8px' }}>
<img src={existingScreenshotUrl} alt="Current screenshot" className="thumb" style={{ width: '50px', height: '50px' }} />
  </div>
)}
...
<textarea name="reasoning" defaultValue={trade.reasoning || ''} placeholder="One or two lines on the read at the time." />
  </div>
