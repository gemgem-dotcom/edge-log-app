...
describe('parseCalendarHtml: days that broke it', () => {
  ...
  // The whole point of sequencing: FF lists one title twice in a day
  // whenever the same official speaks morning and evening, and the
  // day|currency|title key gave both the same identity.
  it('keeps two same-day releases of one title apart', () => {
    const d = dateline('2026-08-28T05:00:00Z')
    const html = row(d, '9:00am', 'FOMC Member Bowman Speaks').replace('data-event-id="1"', 'data-event-id="11"')
      + row(d, '7:30pm', 'FOMC Member Bowman Speaks').replace('data-event-id="1"', 'data-event-id="22"')
    const { events } = parseCalendarHtml(html)
    expect(events).toHaveLength(2)
    expect(events.map((e) => e.event_key)).toEqual([
      '2026-08-28|USD|FOMC Member Bowman Speaks',
      '2026-08-28|USD|FOMC Member Bowman Speaks|#1',
    ])
  })
  ...
  it('still collapses one event that the page lists twice', () => {
    const d = dateline('2026-08-28T05:00:00Z')
    const html = row(d, '9:00am', 'Core PCE') + row(d, '9:30am', 'Core PCE')
    const { events } = parseCalendarHtml(html)
    expect(events).toHaveLength(1)
    expect(events[0].event_key).toBe('2026-08-28|USD|Core PCE')
  })
  ...
  it('files a row under FF\'s day when FF serves a zone ahead of UTC', () => {
    ...
    expect(e.event_key).toBe('2026-07-02|CHF|CPI m/m')
  ...
  it('gives one release the same key from either zone', () => {
    const ahead = parseCalendarHtml(row(dateline('2026-07-01T23:00:00Z'), '7:30am', 'CPI m/m', 'CHF')).events[0]
    const behind = parseCalendarHtml(row(dateline('2026-07-02T05:00:00Z'), '1:30am', 'CPI m/m', 'CHF')).events[0]
