...
// entry side, that leg's own exit price for an exit leg): among every tick
// in the already-fetched, padded set (see FILL_SEARCH_PAD_MINUTES) that
// matches the price, picks whichever is closest in time to the logged
// instant. Unlike the earlier 1-minute-bar version of this search, there's
// no minute-bucket to check in priority order - a real trade print carries
// its own exact timestamp, so "closest match in the padded window" is
// already the most precise answer available. Falls back to roughInstant
// itself (matched: false) if nothing in `ticks` matches - a genuine miss,
// not assumed to be this function's fault.
export function findFillTick({ ticks, roughInstant, price }) {
  let best = null
...
// `ticks` is one already-fetched, padded set covering the whole rough
// window - shared across every anchor rather than re-fetched per leg.
// Returns the derived entry/exit instants ready to feed a slice, plus
// usedFallback: true if entry or any leg couldn't be matched to a real
// trade print and fell back to its raw wall-clock instant - this needs to
// stay visible per trade (schema.sql's `excursion_fallback` column), since
// a trade using the fallback still carries the original second-level
// imprecision this whole mechanism exists to remove.
