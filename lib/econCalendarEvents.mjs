...
// The nine currencies FF's calendar covers, plus 'All'. Not derived from
// whatever happens to be in the table, so the filter list stays put across
// a quiet week that happens to carry no CHF events at all.
//
// 'ALL' is a real value in the feed, not a placeholder: FF files events
// with no single home currency (OPEC meetings, G20 summits) under country
// "All", which normalizeFeedEvent below upper-cases along with every other
// code. Confirmed live - one showed up in the first week's payload. It has
// to be in this list, because the card filters by exact membership, so a
// currency missing from here is an event no checkbox can ever match and
// the trader can never see.
export const CURRENCIES = ['ALL', 'AUD', 'CAD', 'CHF', 'CNY', 'EUR', 'GBP', 'JPY', 'NZD', 'USD']
