...
// mock-DB Turbopack resolveAlias - which matches only the exact
// `@/lib/supabaseClient` specifier string - actually catches this import.
...
async function writeCached(userId, scope, { narrative, generatedAt, tradeCount }) {
  ...
  // but always pass an array here - the mock client only ever handles one.
...
async function callGenerate(scope, tradeCount, previousNarrative) {
  const { data: { session } } = await supabase.auth.getSession()
  const token = session?.access_token
  const res = await fetch('/api/generate-insights', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: 'Bearer ' + token } : {}) },
    body: JSON.stringify({ scope, tradeCount, previousNarrative }),
  })
  if (!res.ok) {
...
// panel's own Generate/Regenerate control, never automatically. Passes
// whatever's currently cached back to Claude as prior context, so a
// regenerate reads as an update to the last write-up rather than a blank
// rewrite each time (explicit trader request for "a level of
// continuity") - the cache read here is otherwise unnecessary, since the
// panel already has last render's narrative in state, but re-reading
// keeps this function self-contained and correct even if called from
// somewhere that hasn't already loaded the cache.
export async function regenerateInsight(scope, currentTradeCount) {
  ...
  const previous = await readCached(user.id, scope)

  try {
    const fresh = await callGenerate(scope, currentTradeCount, previous?.narrative)
